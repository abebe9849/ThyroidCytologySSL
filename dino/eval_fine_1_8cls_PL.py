...
def eval_linear(args):
    ...
    tra_df_original = df[df["fold"]!=FOLD].reset_index(drop=True)

    tra_df_init = tra_df_original.sample(frac=0.01,random_state=int(args.seed)).reset_index(drop=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loader = torch.utils.data.DataLoader(dataset_train,batch_size=args.batch_size_per_gpu,num_workers=args.num_workers,pin_memory=True)
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            if epoch % 5 == 2 and epoch>30:#workではある
                no_label_df = tra_df_original[~tra_df_original["file_path"].isin(tra_df["file_path"].unique())].reset_index(drop=True)

                no_label_df["label"]=0
                print(no_label_df.shape)
                dataset_inf = TrainDataset(no_label_df, transform=val_transform)
                inf_loader = torch.utils.data.DataLoader(dataset_inf,batch_size=args.batch_size_per_gpu*4,num_workers=args.num_workers,pin_memory=True)

                no_label_preds = inference(inf_loader, model, args.n_last_blocks, args.avgpool_patchtokens)
                no_label_df["max"]=np.max(no_label_preds, axis=1)
                no_label_df["label"]=np.argmax(no_label_preds, axis=1)
                add_df = no_label_df[no_label_df["max"]>0.98].reset_index(drop=True)
                print("ADD DF",add_df.shape)
                tra_df = pd.concat([tra_df,add_df]).reset_index(drop=True)
...
@torch.no_grad()
def inference(val_loader, model, n, avgpool):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'INF:'
    preds = []
    valid_labels = []
    softmax= nn.Softmax(dim=1)
    for inp, _ in metric_logger.log_every(val_loader, 20, header):
        # move to gpu
        inp = inp.cuda(non_blocking=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--freeze", default=12, type=int)
    parser.add_argument('--num_workers', default=10, type=int, help='Number of data loading workers per GPU.')
    parser.add_argument('--val_freq', default=1, type=int, help="Epoch frequency for validation.")
    parser.add_argument('--output_dir', default="/home/abe/kuma-ssl/dino/exp002/finetune8/1perPL", help='Path to save logs and checkpoints')
