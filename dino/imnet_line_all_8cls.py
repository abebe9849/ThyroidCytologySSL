...
def train(model, optimizer, loader, epoch, n, avgpool):
    ...
    for (inp, target) in metric_logger.log_every(loader, 80, header):
        # move to gpu
...
class Model(nn.Module):
    ...
    def __init__(self, dim=384, num_labels=1000):
        ...
        self.backbone =  timm.create_model("vit_small_patch16_224_in21k",pretrained=True,num_classes=8)
        self.backbone.eval()
        self.linear = nn.Linear(dim*4, num_labels)
    ...
    def get_emb(self, x):
        ...
        for i in range(len(self.backbone.blocks)):
            ...
            if len(self.backbone.blocks)-4<=i:
                out.append(x[:,0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default="/home/abe/kuma-ssl/dino/imnetViTS_line/all", help='Path to save logs and checkpoints')
