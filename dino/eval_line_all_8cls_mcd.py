...
def eval_linear(args):
    ...
    linear_classifier.load_state_dict(state_dict)
    linear_classifier.eval()
    _,preds,tes_labels = test_network(test_loader, model, linear_classifier, args.n_last_blocks, args.avgpool_patchtokens)
    ...
    for inp, target in metric_logger.log_every(val_loader, 20, header):
        ...
        with torch.no_grad():
            if "vit" in args.arch:
                ...
                output = model(inp)
        output = linear_classifier(output,False)
        loss = nn.CrossEntropyLoss()(output, target)
...
@torch.no_grad()
def test_network(val_loader, model, linear_classifier, n, avgpool):
    linear_classifier.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'INF:'
    
    preds = np.zeros((NUM_drop,25884,8))
    valid_labels = []
    softmax= nn.Softmax(dim=1)
    c = 0
    for inp, target in metric_logger.log_every(val_loader, 20, header):
        ...
        for i in range(NUM_drop):
            output_ = linear_classifier(output,True)
            preds[i,c:c+batch_size,:] = softmax(output_).to('cpu').numpy()
    ...
    print(preds.shape)
    return valid_labels,preds,valid_labels
...
class LinearClassifier(nn.Module):
    ...
    def forward(self, x,train=True):
        ...
        x = torch.nn.functional.dropout(x,0.2, train)
        # linear layer
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default="/home/abe/kuma-ssl/dino/exp002/all8_mcd", help='Path to save logs and checkpoints')
